...
import { ProductWithCategory, Category, StockStatus, VariantGroup } from '@/lib/types';
...
export function ProductFormModal({
  ...
  categories: Category[];
  onSave: (data: Omit<ProductWithCategory, 'id' | 'created_at' | 'updated_at' | 'category'>) => Promise<void>;
  onClose: () => void;
}) {
  ...
  const update = (field: keyof FormData, value: string | boolean | VariantGroup[] | string[]) => {
  ...
  const updateVariant = (index: number, field: keyof VariantGroup, value: string) => {
  ...
  const updateVariantOption = (vIndex: number, oIndex: number, value: string) => {
    const variants = [...form.variants];
  ...
  const removeVariantOption = (vIndex: number, oIndex: number) => {
    const variants = [...form.variants];
    variants[vIndex].options = variants[vIndex].options.filter((_, i) => i !== oIndex);
    update('variants', variants);
  };

  const removeVariant = (vIndex: number) => {
    update('variants', form.variants.filter((_, i) => i !== vIndex));
  };
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (!form.price || parseFloat(form.price) < 0) { setError('Prix invalide.'); return; }
    ...
    try {
      await onSave({
        ...
        variants: form.variants.filter((v) => v.name.trim() && v.options.some((o) => o.trim())),
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/40 backdrop-blur-sm" onClick={onClose}>
      ...
      >
        <div className="sticky top-0 flex items-center justify-between p-5 border-b border-border bg-card z-10">
          <h2 className="text-lg font-bold">{product ? 'Modifier le produit' : 'Nouveau produit'}</h2>
        ...
        <form onSubmit={handleSubmit} className="p-5 space-y-5">
          ...
          {/* Images */}
          <div className="space-y-2">
            <label className="text-sm font-semibold">Images (URLs)</label>
            <div className="space-y-2">
          ...
          <div className="space-y-3">
            ...
            {form.variants.map((variant, vIndex) => (
              <div key={vIndex} className="rounded-xl border border-border p-4 space-y-3 bg-muted/20">
